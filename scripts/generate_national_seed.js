...
async function run() {
  ...
  // Daftar buku yang ditarik / dalam revisi / 404 dari SIBI Kemendikdasmen
  ...
  const validBooks = books.filter(b => {
    if (!b.attachment || !b.title) return false;
    const cleanUrl = b.attachment.trim();
  ...
  console.log(`Valid verified PDF books: ${validBooks.length}`);
