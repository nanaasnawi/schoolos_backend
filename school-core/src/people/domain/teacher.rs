...
impl AggregateRoot for Teacher {
    fn id(&self) -> Uuid {
        self.id
    }
    fn version(&self) -> i32 {
...
impl Teacher {
    pub fn new(tenant_id: Uuid, full_name: String, nip: Option<String>, clock: &dyn Clock) -> Self {
        if tenant_id.is_nil() {
            panic!("Teacher::new called with nil tenant_id");
        }
        if full_name.trim().is_empty() {
            panic!("Teacher::new called with empty full_name");
        }
        let now = clock.now();
        Self {
            id: Uuid::now_v7(),
