    ...
    let target_class_id: Option<Uuid> = match payload.class_id {
        ...
        _ => {
            if let Some(ref desc) = payload.description {
                let parts: Vec<&str> = desc.split(" â€¢ ").collect();
                if parts.len() >= 2 {
...
async fn get_by_id(
    ...
    // Resolve teacher_id if actor is a teacher
    let teacher_id: Option<Uuid> = if is_teacher {
    ...
    // Resolve student_class_id if actor is a student
    let student_class_id: Option<Uuid> = if is_student {
        if let Some(aid) = actor_id {
            sqlx::query_scalar!(
                r#"SELECT class_id FROM students WHERE user_id = $1 AND tenant_id = $2 LIMIT 1"#,
                aid,
                req_ctx.tenant_id
            )
            .fetch_optional(&ctx.pool)
    ...
    // Access control: teachers can only view their own materials, students can only view materials for their class
    if is_teacher {
        if material.teacher_id != teacher_id.unwrap_or_default() {
            return Err(ApiError::new(
                school_core::common::error::ApplicationError::Unauthorized(
                    school_core::common::error_code::ErrorCode::AuthPermissionDenied,
                    "Anda tidak memiliki akses ke materi pembelajaran ini".to_string(),
                ),
                &req_ctx.request_id,
            ));
        }
    } else if is_student {
        if material.class_id != student_class_id {
            return Err(ApiError::new(
                school_core::common::error::ApplicationError::Unauthorized(
                    school_core::common::error_code::ErrorCode::AuthPermissionDenied,
                    "Materi pembelajaran ini bukan untuk kelas Anda".to_string(),
                ),
                &req_ctx.request_id,
            ));
        }
...
async fn toggle_complete(
    ...
    // Access control: verify material belongs to student's class
    let student_class_id: Option<Uuid> = sqlx::query_scalar!(
        r#"SELECT class_id FROM students WHERE user_id = $1 AND tenant_id = $2 LIMIT 1"#,
        actor_id,
        req_ctx.tenant_id
    )
    ...
    let material_class_id: Option<Uuid> = sqlx::query_scalar!(
        r#"SELECT class_id FROM learning_materials WHERE id = $1 AND tenant_id = $2"#,
        id,
        req_ctx.tenant_id
    )
    .fetch_optional(&ctx.pool)
    ...
    if let (Some(sid), Some(mid)) = (student_class_id, material_class_id) {
        if sid != mid {
            return Err(ApiError::new(
                school_core::common::error::ApplicationError::Unauthorized(
                    school_core::common::error_code::ErrorCode::AuthPermissionDenied,
